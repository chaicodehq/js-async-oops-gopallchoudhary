...
export class HaveliSecurity {
  ...
  addResident(name, role, passcode) {
    ...
    if (!["malik", "naukar", "mehman"].includes(role)) return { success: false, message: "Invalid role!" }

    if (this.#residents.some((item) => item.name === name)) return { success: false, message: "Already a resident!" }
  ...
  removeResident(name, passcode) {
    // Your code here
    if (passcode !== this.#passcode) return { success: false, message: "Galat passcode!" }
    if (!this.#residents.some((item) => item.name === name)) return { success: false, message: "Resident nahi mila!" }

    this.#residents.filter((item) => item.name !== name)
  ...
  verifyAccess(name) {
    // Your code here
    const isRegistered = this.#residents.some((item) => item.name === name)
    if (isRegistered) {
  ...
  isResident(name) {
    // Your code here
    const isThere = this.#residents.find((item) => item.name === name)
